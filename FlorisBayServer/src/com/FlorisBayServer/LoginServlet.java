...
//			out.println(json.toString());
			...
			long amount1 = collection.countDocuments(eq("username",username));
			long amount2 = collection.countDocuments(eq("password",password));
			long amount = collection.countDocuments(and(eq("username",username), eq("password",password)));
			if(amount == 0) {
				response.setHeader("result", "fail");
			}else{
				response.setHeader("result", "succes");
			}
